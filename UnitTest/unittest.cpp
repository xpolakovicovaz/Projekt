...
#include <vector>
	...
	//				multi_thread(pt, dlzka, Bdata.Scan0, 0, pBitmap->GetHeight(), Bdata.Stride, pBitmap->GetWidth(), std::ref(HistRed1), std::ref(HistGreen1), std::ref(HistBlue1), std::ref(HistBright1), m_thread_id);
				...
				multi_thread(pt,d,pBitmap, 0, d, d,d, std::ref(HistRed1), std::ref(HistGreen1), std::ref(HistBlue1), std::ref(HistBright1));
				
				for (int qwe = 0; qwe < pt-1; qwe++) {
					Assert::AreEqual(r[255], dlzka*d, L"cerveny");
					Assert::AreEqual(g[88], dlzka*d, L"zeleny");
					Assert::AreEqual(b[0], dlzka*d, L"modry");
					Assert::AreEqual(j[(int)(0.2126 * 255 + 0.7152 * 88 + 0.0722 * 0)], dlzka*d, L"jas");
					for (int i = 0; i < 256; i++)
					{
				...
				Assert::AreEqual(r[255], dlzka*(d-(pt-1)*dlzka), L"cerveny");
				Assert::AreEqual(g[88], dlzka*(d - (pt - 1)*dlzka), L"zeleny");
				Assert::AreEqual(b[0], dlzka*(d - (pt - 1)*dlzka), L"modry");
				Assert::AreEqual(j[(int)(0.2126 * 255 + 0.7152 * 88 + 0.0722 * 0)], dlzka*(d - (pt - 1)*dlzka), L"jas");
				for (int i = 0; i < 256; i++)
				{
					if (i != 255) Assert::AreEqual(r[i], 0, L"cerveny0");
					if (i != 88) Assert::AreEqual(g[i], 0, L"zeleny0");
